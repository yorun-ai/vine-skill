...
- The default call timeout in `v0.12.0` is 30 seconds. Use generated capabilities or public invocation options when another value is needed, and verify the target version.
...
Defaults and limits in `v0.12.0`:
...
- Do not rely on global ordering, replay for late Listeners, audit history, exactly-once delivery, or message retention across runtime restarts.
- If the business cannot tolerate loss on runtime restart, use a database outbox, durable workflow, or another business record.
...
- Use standalone/testkit for business behavior and serialization. Use separated processes for NATS, Link, leases, and networking.
